...
class MickeysQRMenu {
    ...
    public function enqueue_frontend_assets() {
        wp_register_style('mickeys-style', MICKEYS_QR_MENU_URL . 'style.css', array(), '1.6.0');
        wp_register_script('mickeys-menu-data', MICKEYS_QR_MENU_URL . 'menu-data.js', array(), '1.6.0', true);
        wp_register_script('mickeys-app', MICKEYS_QR_MENU_URL . 'script.js', array('mickeys-menu-data', 'jquery'), '1.6.0', true);
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_enqueue_style('mickeys-admin-style', MICKEYS_QR_MENU_URL . 'admin-style.css', array(), '1.6.0');
        ...
        wp_enqueue_script('mickeys-menu-data', MICKEYS_QR_MENU_URL . 'menu-data.js', array(), '1.6.0', true);
        wp_enqueue_script('mickeys-admin-script', MICKEYS_QR_MENU_URL . 'admin-script.js', array('jquery', 'jquery-ui-sortable', 'xlsx', 'mickeys-menu-data'), '1.6.0', true);
    ...
    public function render_frontend_menu($atts) {
        ...
        if (!$theme_settings) $theme_settings = array();
    ...
    public function print_dynamic_styles() {
        ...
        if ($settings) {
            ...
            if ($settings) {
                ...
                $fonts = array();
                if (!empty($settings['fontPrimary'])) $fonts[] = $settings['fontPrimary'];
                if (!empty($settings['fontDisplay'])) $fonts[] = $settings['fontDisplay'];
                
                if (!empty($fonts)) {
                    $fonts = array_unique($fonts);
                    $font_family_strings = array();
                    foreach ($fonts as $font) {
                        if ($font === 'Metropolis') continue; // Skip non-Google
                ...
                // Fonts
                if (!empty($settings['fontPrimary'])) echo '--font-primary: \'' . esc_attr($settings['fontPrimary']) . '\', sans-serif !important;';
                if (!empty($settings['fontDisplay'])) echo '--font-display: \'' . esc_attr($settings['fontDisplay']) . '\', serif !important;';
                
                // Colors
                if (!empty($settings['primaryColor'])) echo '--primary-color: ' . esc_attr($settings['primaryColor']) . ';';
                if (!empty($settings['secondaryColor'])) echo '--secondary-color: ' . esc_attr($settings['secondaryColor']) . ';';
                if (!empty($settings['bgColor'])) echo '--bg-body: ' . esc_attr($settings['bgColor']) . ';';
                if (!empty($settings['cardColor'])) echo '--bg-card: ' . esc_attr($settings['cardColor']) . ';';
                if (!empty($settings['textColor'])) echo '--text-primary: ' . esc_attr($settings['textColor']) . ';';
                
                // Footer Colors
                if (!empty($settings['footerHeaderColor'])) echo '--footer-header-color: ' . esc_attr($settings['footerHeaderColor']) . ';';
                if (!empty($settings['footerTextColor'])) echo '--footer-text-color: ' . esc_attr($settings['footerTextColor']) . ';';
                ...
                if (isset($settings['layoutWidth'])) {
                    if ($settings['layoutWidth'] === 'container') $maxWidth = '1200px';
                    if ($settings['layoutWidth'] === 'narrow') $maxWidth = '800px';
                }
                ...
                // Width constraints
                echo '.container { max-width: var(--container-max-width) !important; margin: 0 auto; }';
