// Frontend Script - Mickey QR Menu
// Debug mode - production'da false olacak
var MICKEY_DEBUG = (typeof mickeyData !== 'undefined' && mickeyData.debug) || false;
...
function openAllergenModal() {
    debugLog('Opening allergen modal...');
    if (allergenModal) {
        allergenModal.classList.add('active');
        document.body.classList.add('no-scroll');
    } else {
        debugLog('allergen Modal not found!');
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    debugLog('DOMContentLoaded - Initializing menu...');
    ...
    if (allergenBtn) {
        ...
        debugLog('Allergen button click handler added');
    }
    ...
    if (typeof mickeyData !== 'undefined') {
        debugLog('WordPress environment detected, loading data via AJAX...');

        jQuery.when(
            jQuery.get(mickeyData.ajax_url, { action: 'mickey_get_data', key: 'mickeys_menu_data', nonce: mickeyData.nonce }),
            jQuery.get(mickeyData.ajax_url, { action: 'mickey_get_data', key: 'mickeys_categories', nonce: mickeyData.nonce }),
            jQuery.get(mickeyData.ajax_url, { action: 'mickey_get_data', key: 'mickeys_allergens', nonce: mickeyData.nonce })
        ).done(function (resMenu, resCat, resAllergen) {
            var menuRes = resMenu[0];
            var catRes = resCat[0];
            var algRes = resAllergen[0];

            debugLog('AJAX responses received');

            if (menuRes.success && menuRes.data) {
                try {
                    menuData = JSON.parse(menuRes.data);
                    debugLog('Menu data loaded:', menuData.length, 'items');
                } catch (e) {
            ...
            if (catRes.success && catRes.data) {
                try {
                    ...
                    if (loadedCategories && Object.keys(loadedCategories).length > 0) {
                        ...
                        debugLog('Categories loaded:', Object.keys(categoryConfig).length, 'categories');
                    }
                } catch (e) {
            ...
            if (algRes.success && algRes.data) {
                try {
                    ...
                    debugLog('Allergens loaded:', adminAllergens.length, 'allergens');
                } catch (e) {
        ...
        debugLog('Non-WordPress environment, using fallback data');
        renderMenuAccordion();
        renderAllergens();
        activateFirstCategory();
    }
