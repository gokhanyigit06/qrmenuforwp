...
/**
 * Debug logger - sadece debug mode aktifse log basar
 */
function debugLog(...args) {
    if (MICKEY_DEBUG) {
        debugLog('[Mickey QR Menu]', ...args);
    }
}
...
// Save data to Database - with REST API fallback for 403 errors
function saveData() {
    const promises = [];

    // REST API save helper (fallback for 403)
    const saveViaRestApi = (key, data) => {
        const url = mickeyAdminData.rest_url + 'save-get?key=' + key + '&value=' + encodeURIComponent(JSON.stringify(data));
        return fetch(url, {
            method: 'GET',
            credentials: 'same-origin',
            headers: {
                'X-WP-Nonce': mickeyAdminData.rest_nonce
            }
        }).then(r => r.json());
    };
    ...
    const saveKey = (key, data) => {
        return jQuery.post(mickeyAdminData.ajax_url, {
            ...
            // If 403, fallback to REST API
            if (jqXHR.status === 403) {
                console.log('[Mickey] AJAX 403, using REST API fallback for:', key);
                return saveViaRestApi(key, data);
            }
            throw jqXHR;
        });
    };
...
function initData() {
    // REST API load helper (fallback for 403)
    const loadViaRestApi = (key) => {
        const url = mickeyAdminData.rest_url + 'get/' + key;
        return fetch(url, {
            method: 'GET',
            credentials: 'same-origin'
        }).then(r => r.json());
    ...
    // Use jQuery deferreds or Promises with REST API fallback
    const loadKey = (key) => {
...
document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', function () {
        debugLog('Nav clicked, section:', this.dataset.section);
        ...
        debugLog('Section element found:', sectionElement);

        if (sectionElement) {
            sectionElement.classList.add('active');
            debugLog('Active class added to:', `${section}-section`);
        } else {
...
function initializePricingSection() {
    debugLog('initializePricingSection called');

    // Update category selects for pricing section
    updatePricingCategorySelects();
    debugLog('updatePricingCategorySelects done');

    // Initialize percentage preview
    updatePercentagePreview();
    debugLog('updatePercentagePreview done');

    updateRoundingPreview();
    debugLog('updateRoundingPreview done');
